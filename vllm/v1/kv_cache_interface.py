...
class TQSlidingWindowSpec(SlidingWindowSpec):
    ...
    For TurboQuant SWA layers: memory allocation respects the sliding window
    limit (like SlidingWindowSpec) while the page size uses TQ slot bytes
    instead of the standard head_size * dtype_size formula.
    """

    tq_slot_size: int = 0  # Must be > 0; 0 only exists as dataclass default

    def __post_init__(self) -> None:
        # Catch misconfiguration early: tq_slot_size=0 would silently fall back
        # to bf16 page sizes, causing KV cache memory layout corruption.
        if self.tq_slot_size <= 0:
            raise ValueError(
                f"TQSlidingWindowSpec requires tq_slot_size > 0, "
                f"got {self.tq_slot_size}. Pass tq_slot_size=tq_config.slot_size_aligned."
            )
